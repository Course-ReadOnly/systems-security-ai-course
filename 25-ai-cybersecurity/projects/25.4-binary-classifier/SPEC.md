...
# Project 25.4 — AI Binary Classifier

## Goal

Push Stage 21.2's classical-ML malware classifier toward Stage 22-style
...
1. A deep-learning model (not classical ML — that's 21.2) classifying
   malicious/benign using features learned from raw binary data, not
   fully hand-engineered ones.
2. Same rigor as 21.2/22.1: real train/test split, precision/recall/F1,
   stated class balance.
3. **Directly compares against your 21.2 classical model on the same
...
- [ ] Precision/recall/F1 on a held-out test set, compared side-by-side
      against 21.2's results on the same split
- [ ] Honest discussion: did deep learning actually help here, or was
      21.2's simpler model comparable/better? Either answer is fine —

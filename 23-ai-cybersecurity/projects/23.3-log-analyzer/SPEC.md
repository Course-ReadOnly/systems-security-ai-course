...
# Project 23.3 — AI Log Analyzer

## Goal

Extend Stage 21.2's log-anomaly-detector and 22.3's threat-hunting
...
1. Pipeline: 21.2's (or an extended) anomaly-detection model flags
   ...
   in the actual log content (RAG-style, same discipline as 22.3).
...
3. Evaluated on real or realistic log data with known anomalies (reuse
   21.2's evaluation data), checking both detection quality (precision/
   recall, from 21.2) and explanation quality (do the explanations
...
actually adding value over the raw detection signal from 21.2 alone.
